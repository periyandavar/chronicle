<?php
class admin_ctrl1 extends CI_Controller
{  
...
$this->load->view('adminsam2',$result);
        ...
        $crud->display_as('Evidence2','Certificate/Evidence');
...
$crud->field_type('Event','dropdown',array('Seminar'=>'Seminar','Conference'=>'Conference','others'=>'others'));
       $crud->columns('Staff_Name','From_Date','To_Date','Level','Event','Title_of_Program','Title_of_Paper','Place','Certificate','Evidence2','Evidence3');
       $state = $crud->getState();
        if ($state == 'export' || $state == 'print') {
    ...
    public function workshop()
    {
        ...
        $_SESSION['CustomTitle']='<br><h3> <center> '.$Department.' </center><br><br> Workshops Attended by Staff </h3>';     
        $crud->set_rules('Date','Date','callback_check_dates[Date]');
       //$crud->where('Status','Approved');
       // $crud->unset_print();
    ...
    $crud->set_rules('To_Date','To_Date','callback_check_dates2[To_Date]');
       ...
       // $crud->unset_print();
        $result['data']=$this->selection_model->disp1($Staff_ID);
        $crud->field_type('Status','hidden','');
        $result['title']="<html><h2><center><b> Workshops Attended </b></h2></html>";
$this->load->view('adminsam2',$result);
$crud->set_field_upload('Evidence2','assets/uploads/files/seminar');
$crud->set_field_upload('Evidence3','assets/uploads/files/seminar');
//$crud->field_type('Presented','dropdown',array('yes'=>'yes','no'=>'no'));
...
$crud->required_fields('Staff_ID','Staff_Name','Duration_of_project','Name_of_Project','Amount_fund_received','Name_of_fundingagency','Year_of_sanction','Certificate');
    ...
    redirect('select_ctrl/header');
        ...
        //$crud->where('Department',$Department);
        $crud->columns('Staff_Name','Duration_of_project','Name_of_Project','Amount_fund_received','Name_of_fundingagency','Year_of_sanction','Certificate','Evidence2','Evidence3');
     ...
     public function Awards()
    {
        ...
        $crud->where('Nature_of_Award!="no"');
          //$crud->unset_export();
          $crud->fields('Staff_ID','Staff_Name','From_Date','Nature_of_Award','Place','Certificate','Evidence3','Evidence2');
          $crud->display_as('Place','Awarding Agency');
          $crud->display_as('From_Date','Date');
          $crud->required_fields('Staff_ID','Staff_Name','Place','From_Date','Nature_of_Award','Awarding_Agency','Date','Certificate','Department');
...
$result['title']="<html><h2><center><b>Project Pro Applied - Grant Applied</b></h2></html>";
 ...
 $this->load->view('adminsam2',$result);
    ...
    if($Department=='')
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
        $_SESSION['CustomTitle']='<br><h3> <center> <br>'.$Department.' </center><br><br>Project Applied (student project) </h3>';
        $crud->where('Department',$Department);
...
public function specialprg()
    ...
    public function book()
    {
       ...
       //$crud->where('Status','Approved');
        ...
        $crud->columns('Staff_ID','Staff_Name','Date','Title_of_book','Type','Issn_isbn_for_proceeding','Year_of_publishing','Certificate','Evidence2','Evidence3',"Status");
        ...
        $state = $crud->getState();
        if ($state == 'export' || $state == 'print') {
            $crud->columns('Staff_ID','Staff_Name','Date','Title_of_book','Type','Issn_isbn_for_proceeding','Year_of_publishing');
        }
...
$crud->field_type('Level','dropdown',array("Internal"=>'Internal',"District"=>"District","Zonal"=>"Zonal","State"=>'State',"National"=>"National","International"=>'International'));
        ...
        $crud->columns('Staff_ID','Staff_Name','Name_of_Journal','Year_Date','Title_of_Paper','ISSN_ISBN','Indexed','Page_No','Impact_Factor','Certificate','Evidence2','Evidence3','Status');
...
public function refresher_orientation()
    {
        ...
        $result['title']= "<html><h2><center><b>Refresh Orientation</center></b></h2></html>";
        $result['data']=$this->selection_model->disp1($Staff_ID);
    $this->load->view('adminsam2',$result);
