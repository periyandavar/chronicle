<?php
class stud_ctrl2 extends CI_Controller
{  
     ...
     public function intercollegiate()
    {
      ...
      $crud=new Grocery_CRUD_MultiSearch();
        $Staff_ID=$this->session->userdata('Staff_ID');
          $result=$this->selection_model->disp1($Staff_ID);
        $Department=$this->session->userdata('Department');
        $_SESSION['CustomTitle']='<br><h3> <center> </center><br><br>Inter-Collegiate Meets Attended by Students</h3>';     
...
//  $crud->set_read_fields('Department','Student_ID','Student_Name','Class','Title_of_program','Level','Organizer','Date','Year','Place_Prize','Certificate','Evidence2','Evidence2','Presented');
  $crud->display_as('Title_of_Paper','Title of the Paper (if Presented)');
        ...
        //$crud->unset_print();
        $_SESSION['CustomTitle']='<br><h3> <center> </center><br><br> Workshops Attended by Students</h3>';     
        //$crud->unset_export();
        $crud->columns('Student_ID','Student_Name','Class','Department',"Date",'To_Date','Level','Title_of_program','Organizer','Place_Prize');
  ...
  $this->load->view('admin_nav',$result);
  $_SESSION['CustomTitle']='<br><h3> <center> </center><br><br>Seminars/Conferences Attended by Students</h3>';     
 ...
 // $crud->set_read_fields('Department','Student_ID','Student_Name','Class','Title_of_program','Place_Prize','Title_of_Paper','Level','Organizer','Date','Year','Certificate','Evidence2','Evidence3');
  $crud->field_type('Level','dropdown',array("Internal"=>'Internal',"District"=>"District","Zonal"=>"Zonal","State"=>'State',"National"=>"National","International"=>'International'));
        $crud->where('(Event_Type="Conference" or Event_Type="Seminar") and Department=""');
       // $crud->unset_print();
       $crud->field_type('Presented','dropdown',array('yes'=>'yes','no'=>'no'));
        //$crud->unset_export();
        ...
        $crud->columns('Student_ID','Student_Name','Class','Department','Date','To_Date','Level','Event_Type','Title_of_program','Organizer','Place_Prize','Presented','Title_of_Paper','Page_No','ISSN','Award','Certificate','Evidence2','Evidence3');
 ...
 // $crud->unset_read_fields('Titleoftheproject','nameofguide','designation','beneficiary','venue','time','year','Competitive_Exam_Passed','traininsitiut','To_Date');
  ...
  public function project_applied()
    {
        ...
        $crud->field_type("Status",'hidden',"Approved");
        $_SESSION['CustomTitle']="<br><h3> <center> </center><br><br>Student's Final Year Projects</h3>";     
        if($Department=='')
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
       // $crud->unset_print();
  ...
  //$crud->set_read_fields('Titleoftheproject','nameofguide','designation','beneficiary','venue','time','year','Competitive_Exam_Passed','traininsitiut','To_Date');
       ...
       // $crud->unset_print();
        //$crud->unset_export();
        $crud->columns('Student_Name','Department','Class','nameofguide','Titleoftheproject','Certificate','Evidence2','Evidence3');
        ...
        $state=$crud->getState();
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Certificate','Evidence2','Evidence1');}
  ...
  $crud->set_field_upload('Evidence3','assets/uploads/files/peer');
       ...
       // $crud->unset_print();
        ...
        $crud->display_as('Organizer','Organized by');
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center> </center><br><br>Peer Learning</h3>';     
        if ($state == 'export' || $state == 'print') {
  ...
  $crud->set_field_upload('Evidence3','assets/uploads/files/comp');
        ...
        $_SESSION['CustomTitle']='<br><h3> <center> </center><br><br>Competitive Exams Passed</h3>';     
        $crud->where('Event_Type','competitive Exam');
     //   $crud->unset_print();
       // $crud->unset_export();
        $crud->columns('Student_ID','Student_Name','Department','Class','Competitive_Exam_Passed','year');
        ...
        //$crud->set_theme('datatables');
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center> </center><br><br>Cultural Meet Attended by Students</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Certificate','Evidence2','Evidence3','Status');}
        $output=$crud->render();
        $this->load->view('studrpt1');
  ...
  public function sports()
    {             
        ...
        //$crud->set_rules('Year','Year','callback_check_year[Year]');
        $crud->field_type('Status','hidden','');
        $_SESSION['CustomTitle']='<br><h3> <center> </center><br><br>Sports Achievement by Students</h3>';     
        if($Department=='')
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
      //  $crud->unset_print();
  ...
  $crud->set_field_upload('Evidence3','assets/uploads/files/sports');
     ...
     $crud->required_fields('Department','Student_ID','Student_Name','Title_of_program','Event_Type','Title_of_Event','Level','Organizer','Date','Year','Certificate');
        $crud->columns('Student_ID','Student_Name','Department','Class','Date','Level','Title_of_program','Title_of_Event','Organizer','Place_Prize','Certificate','Evidence2','Evidence3');
  ...
  $crud->field_type('Event_Type','hidden','Cultural Competition');
    //    $crud->unset_print();
     ...
     //   $crud->set_read_fields('Department','Student_ID','Event_Type','Title_of_program','Title_of_Event','Place_Prize','Level','Organizer','Date','Year','Certificate','Evidence2','Evidence3');
        ...
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center> </center><br><br>Cultural Meets Attended by Students</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Certificate','Evidence2','Evidence3','Status');}
        $output=$crud->render();
        $this->load->view('studrpt1');
  ...
  $crud->set_field_upload('Evidence3','assets/uploads/files/placement');
        ...
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center> </center><br><br>Placement</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Evidance','Evidence2','Evidence3','Status');}
        $output=$crud->render();
        $this->load->view('studrpt1');
           $this->load->view('table',$output);
    }
