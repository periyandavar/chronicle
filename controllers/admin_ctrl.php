...
/*if(!empty($this->input->post('Staff_ID')){
        ...
        public function NME()
    {
     ...
     $state=$crud->getState();
        ...
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Evidence1','Evidence2','Evidence3','Status','Department');}
     $output=$crud->render();
    ...
    public function state()
    {
     ...
     $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Physically Disabled Students</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Evidence1','Evidence2','Evidence3','Status','Department');}
     $output=$crud->render();
    ...
    public function phd()
    {
       ...
       $crud->display_as('Evidence2','Circular/Evieance');
       $crud->display_as('Evidence3','Circular/Evidence');
    ...
    public function guide()
    {
        ...
        $crud->required_fields('Staff_ID','Staff_Name','Co_Guide','Student_Name','Title','Date_of_Submission','Date_of_Viva_Voce','Date_of_Syndicate_Approval_letter','Mentioned','Evidence');
        if($Department=='')
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
    ...
    public function exam()
    {
        ...
        $crud=new Grocery_CRUD_MultiSearch();
        $crud->set_table('exam');
        $_SESSION['CustomTitle']='<br><h3> <center> '.$Department.' </center><br><br> Exams Passed By Staff (NET/SET) </h3>';    
        $state=$crud->getState();
       if ($state == 'export' || $state == 'print') {
        $crud->unset_columns('Evidence','Evidence2','Evidence3','Status','Department');}
        ...
        $crud->field_type("Department",'hidden',$Department);
        $result['data']=$this->selection_model->disp1($Staff_ID);
        $result['title']="<html><h2><center> <b> Exams Passed (NET/SET) </b></h2></html>";
        $this->load->view('adminsam1',$result);
        $this->load->view('staff');
        $crud->unset_fields('Status');
        //$crud->set_theme('datatables');
    ...
    public function study()
    {
        ...
        $_SESSION['CustomTitle']='<br><h3> <center> '.$Department.' </center><br><br> Board of Studies Attended by faculty </h3>';    
    ...
    public function board()
    {
        ...
        $crud->unset_add();
        $crud->set_rules('Year','Year','callback_check_year1[Year]');
        $_SESSION['CustomTitle']='<br><h3> <center> '.$Department.' </center><br><br> Board of Studies</h3>';    
        $crud->set_rules('Joining_Year','Joining_Year','callback_check_year1[Joining_Year]');
        if($Department=='')
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
    ...
    public function assosiate()
    {
       ...
       $crud->columns('Department','Event','Date','Strength','Staff_Incharge','Winner','Runner','Evidence1','Evidence2','Evidence3');
        ...
        $crud->set_field_upload('Evidence3','assets/uploads/files/Assosiation');
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Assosiation Meetings</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Evidence1','Evidence2','Evidence3','Status','Department');}
        $this->load->view('depart');
        $output=$crud->render();
...
public function training(){
    ...
    $crud->where("(Event_Type='Training Program' or Event_Type='Internship Training Program') and Department='".$Department."'");
    //$crud->unset_print();
    $result['data']=$this->selection_model->disp1($Staff_ID);
...
$crud->set_field_upload('Evidence3','assets/uploads/files/internship');
   ...
   $crud->columns('Student_ID','Student_Name','Class','Department','Date','To_Date','time','Event_Type','Title_of_Event','Organizer','year','Certificate','Evidence2','Evidence3');
    ...
    $state=$crud->getState();
    $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Internship/Trainning Programs</h3>';     
        if ($state == 'export' || $state == 'print') {
...
$crud->set_field_upload('Evidence3','assets/uploads/files/paper');
	...
	public function guest_lecture()
    {
       ...
       $_SESSION['CustomTitle']='<br><h3> <center> '.$Department.' </center><br><br> Guest Lectures Delivered by Staff </h3>';    
        $crud->where('Department',$Department);
        if($Department=='')
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
...
$this->load->view('adminsam1',$result);
    ...
    //     $crud->set_subject('Seminar/Conference');
        $_SESSION['CustomTitle']='<br><h3> <center> '.$Department.' </center><br><br> Seminars/Conferences Attended by Staff </h3>';     
        $crud->set_rules('Date','Date','callback_check_dates[Date]');
      // $crud->where('Status','Approved');
       // $crud->unset_print();
       $crud->where('(Event="Seminar" or Event="Conference") and Department="'.$Department.'"');
    ...
    $crud->set_rules('To_Date','To_Date','callback_check_dates2[To_Date]');
       ...
       // $crud->unset_print();
        $result['data']=$this->selection_model->disp1($Staff_ID);
        $crud->field_type('Status','hidden','');
        $result['title']="<html><h2><center><b> Seminars/Conferences Attended </b></h2></html>";
$this->load->view('adminsam1',$result);
...
$crud->field_type('Nature_of_Award','dropdown',array('Best Paper Award'=>'yes','no'=>'no'));
...
$crud->field_type('Event','dropdown',array('Seminar'=>'Seminar','Conference'=>'Conference','others'=>'others'));
       $crud->columns('Staff_Name','From_Date','To_Date','Level','Event','Title_of_Program','Title_of_Proceeding','Title_of_Paper','Place','Certificate','Evidence2','Evidence3');
       $state = $crud->getState();
        if ($state == 'export' || $state == 'print') {
            $crud->columns('Staff_Name','From_Date','To_Date','Level','Event','Title_of_Program','Title_of_Paper','Place');
        }
       $crud->fields('Staff_ID','Status','Staff_Name','Department','From_Date','To_Date','Level','Event','Title_of_Program','Title_of_Proceeding','Place','Presented','Title_of_Paper','Page_No','ISSN','Mentioned','Nature_of_Award','Certificate','Evidence2','Evidence3');
      $crud->required_fields('Staff_ID','Staff_Name','Department','Level','Event','Title_of_Program','Title_of_Proceeding','Presented','Nature_of_Award','Place','From_Date','Certificate');
      $crud->display_as('Mentioned','Name of the College Mentioned Yes/No');
       ...
       $crud->display_as('Nature_of_Award','Secure Best Paper Award..?');
    ...
    public function workshop()
    {
        ...
        $_SESSION['CustomTitle']='<br><h3> <center> '.$Department.' </center><br><br> Workshop Attended by Staff </h3>';     
        $crud->set_rules('Date','Date','callback_check_dates[Date]');
       //$crud->where('Status','Approved');
       // $crud->unset_print();
    ...
    $crud->set_rules('To_Date','To_Date','callback_check_dates2[To_Date]');
       ...
       // $crud->unset_print();
        $result['data']=$this->selection_model->disp1($Staff_ID);
        $crud->field_type('Status','hidden','');
        $result['title']="<html><h2><center><b> workshop Attended </b></h2></html>";
$this->load->view('adminsam1',$result);
$crud->set_field_upload('Evidence2','assets/uploads/files/seminar');
$crud->set_field_upload('Evidence3','assets/uploads/files/seminar');
//$crud->field_type('Presented','dropdown',array('yes'=>'yes','no'=>'no'));
//$crud->field_type('Best_Pape_Award','dropdown',array('yes'=>'yes','no'=>'no'));
$crud->field_type('Level','dropdown',array("Internal"=>'Internal',"District"=>"District","Zonal"=>"Zonal","State"=>'State',"National"=>"National","International"=>'International'));
$crud->field_type('Event','hidden','Workshop');
$crud->columns('Staff_ID','Staff_Name','From_Date','To_Date','Level','Event','Title_of_Program','Place','Certificate','Evidence2','Evidence3');
       $state = $crud->getState();
        if ($state == 'export' || $state == 'print') {
            $crud->columns('Staff_ID','Staff_Name','From_Date','To_Date','Level','Event','Title_of_Program','Place');
        }
       ...
       $crud->required_fields('Staff_ID','Staff_Name','Department','Level','Event','Title_of_Program','Presented','Nature_of_Award','Place','From_Date','Certificate');
       $crud->display_as('Title_of_Paper','Title of the Paper (if presented)') ;
       //$crud->set_theme('datatables');
       $crud->display_as('Nature_of_Award','Secure Best Paper Award..?');
        $crud->set_field_upload('Certificate','assets/uploads/files/Seminar');
    ...
    {
         if($post_array['Nature_of_Award']=='yes')
       { $user_logs_insert = array(
            "Staff_ID" => ($post_array['Staff_ID']),
            "Staff_Name"=>($post_array['Staff_Name']),
            "Department"=>($post_array['Department']),
            "Awarding_Agency"=>($post_array['Place']),
            'Date'=>date_format(new DateTime($post_array['From_Date']),'Y/m/d'),
            // "Date"=>$post_array['From_Date'],
...
redirect('select_ctrl/header');
$crud->field_type('Department','hidden',$Department);
$crud->display_as('Nature_of_Project','Nature of the Research Project (Minor/Major)');
...
$crud->required_fields('Staff_ID','Staff_Name','Nature_of_Project','Date_of_Implementation','Co_Investigator','Duration_of_project','Name_of_Project','Received','Name_of_fundingagency','Year_of_sanction','Certificate');
    if($Department=='')
    redirect('select_ctrl/header');
         $crud->set_subject('Research Project Received');
        $crud->where('Department',$Department);
        $crud->columns('Staff_Name','Co_Investigator','Duration_of_project','Name_of_Project','Nature_of_Project','Date_of_Implementation','Amount_fund_received','Name_of_fundingagency','Year_of_sanction','Certificate','Evidence2','Evidence3');
        $_SESSION['CustomTitle']='<br><h3> <center> <br>'.$Department.' </center><br><br>Research Project Received</h3>';     
        $state=$crud->getState();
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Certificate','Evidence2','Evidence3','Status','Department');}
...
$this->load->view('adminsam1',$result);
      ...
      $crud->where('Nature_of_Award!="no"');
        //$crud->unset_export();
        $crud->fields('Staff_ID','Staff_Name','From_Date','Nature_of_Award','Level','Place','Certificate','Evidence3','Evidence2','Department');
        $crud->columns('Staff_ID','Staff_Name','From_Date','Nature_of_Award','Level','Place','Certificate','Evidence3','Evidence2','Department');
        $crud->display_as('Place','Awarding Agency');
        $crud->display_as('From_Date','Date');
        $crud->required_fields('Staff_ID','Staff_Name','Place','From_Date','Nature_of_Award','Awarding_Agency','Date','Certificate','Level','Department');
        ...
        $crud->set_field_upload('Certificate','assets/uploads/files/seminar');
        $crud->set_field_upload('Evidence3','assets/uploads/files/seminar');
        $_SESSION['CustomTitle']='<br><h3> <center> '.$Department.' </center><br><br> Awards Received by Staff </h3>';    
          $state=$crud->getState();
         if ($state == 'export' || $state == 'print') {
          $crud->unset_columns('Certificate','Evidence2','Evidence3','Status','Department');}
        $crud->set_field_upload('Evidence2','assets/uploads/files/seminar');
...
$this->load->view('adminsam1',$result); 
    ...
    $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>E-Contents Prepared</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Certificate','Evidence2','Evidence3','Status','Department');}
    $output=$crud->render();
    $this->load->view('depart');
    ...
    public function grant_applied()
    {
        ...
        $crud->field_type('Received','dropdown',array('Yes'=>'Yes','No'=>'No'));
        $crud->columns('Organizing_Secretary','Level','Event_Name','Title','Fund_expected','Agency','Received','Date');
        $_SESSION['CustomTitle']='<br><h3> <center> <br>'.$Department.' </center><br><br> Project Pro Applied (Grant)</h3>';
        if($Department=='')
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
        ...
        $crud->set_field_upload('Evidence2','assets/uploads/files/project');
        $crud->set_field_upload('Evidence3','assets/uploads/files/project');
        $crud->required_fields('Staff_ID','Organizing_Secretary','Level','Event_Name','Title','Fund_expected','Received','Agency','Evidence');
        $state=$crud->getState();
        if ($state == 'export' || $state == 'print') {
           $crud->unset_columns('Evidence','Evidence2','Evidence3','Status','Department');}
        $output=$crud->render();
...
$result['title']="<html><h2><center><b>Project Proposals Applied - Grant Applied</b></h2></html>";
    ...
    public function first()
    {
        ...
        $crud->where('Department',$Department);
        $crud->field_type('Department','hidden',$Department);
        $_SESSION['CustomTitle']='<br><h3> <center><br>'.$Department.' </center><br><br>First Graduate</h3>';     
 ...
 $this->load->view('adminsam1',$result);
    ...
    public function overall_result()
    {
        ...
        $crud->field_type('Department','hidden',$Department);
        $output=$crud->render();
        $_SESSION['CustomTitle']='<br><h3> <center><br>'.$Department.' </center><br><br>Overall Results</h3>';     
        $result['data']=$this->selection_model->disp1($Staff_ID);
        $crud->unset_add();
        $crud->unset_edit();
        $crud->unset_delete();
 ...
 $this->load->view('adminsam1',$result);
    ...
    public function drop()
    {
        ...
        $crud->where('Department',$Department);
        $crud->field_type('Department','hidden',$Department);
        $_SESSION['CustomTitle']='<br><h3> <center><br>'.$Department.' </center><br><br>Student Dropouts</h3>';     
 ...
 $this->load->view('adminsam1',$result);
    ...
    public function Project_pro()
    {
        ...
        $Staff_ID=$this->session->userdata('Staff_ID');
    $result['data']=$this->selection_model->disp1($Staff_ID);
    $result['title']="<html><h2><center><b>Project Proposals Applied - Student Projects </b></h2></html>";
    $this->load->view('adminsam1',$result);
    $crud->set_field_upload('Evidence','assets/uploads/files/project');
        $crud->set_field_upload('Evidence2','assets/uploads/files/project');
        $crud->set_field_upload('Evidence3','assets/uploads/files/project');
    if($Department=='')
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
        $_SESSION['CustomTitle']='<br><h3> <center> <br>'.$Department.' </center><br><br>Projects Applied (student project) </h3>';
        $crud->where('Department',$Department);
        // $crud->columns('Date','Event_Name','Title','Fund_expected','Agency');
        $crud->required_fields('Period_of_Project','Title_of_Project','Student_Name','Staff_ID','Staff_Name','Date','Agency','Amount','Evidence');
        $state=$crud->getState();
        // $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>E-Contents Prepared</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Evidence','Evidence2','Evidence3','Status','Department');}
$output=$crud->render();
    ...
    public function work()
    {
        ...
        $crud->columns('Staff_ID','Staff_Name','Year','UG_Gain','UG_Loss','[UG] (+) or (-) ','PG_Gain','PG_Loss','[PG] (+) or (-) ');
       ...
       // $crud->display_as('POMU','[UG] (+) or (-) ');
        ...
        //$crud->unset_delete();
        //$crud->unset_read();
        $_SESSION['CustomTitle']='<br><h3> <center><br>'.$Department.' </center><br><br>Work Adjustment</h3>';
        $output=$crud->render();
        $result['data']=$this->selection_model->disp1($Staff_ID);
...
public function add1($value, $row)
{
    ...
    public function remed()
    {
        ...
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Remedial Class </h3>';
        if($Department=='')
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
...
$Staff_ID=$this->session->userdata('Staff_ID');
    ...
    public function FDP()
    {
        ...
        $state = $crud->getState();
        if ($state == 'export' || $state == 'print') {
            $crud->unset_columns('Certificate','Evidence2','Evidence3','Department','Status');
        }
...
$crud->set_field_upload('Evidence3','assets/uploads/files/FDP');
        ...
        $_SESSION['CustomTitle']='<br><h3> <center> <br>'.$Department.' </center><br><br>Faculty Development Programs Attended by Staff</h3>';     
...
$crud->required_fields('Date','Name_of_The_Staff','Date','Name_of_Project','Name_of_Agency','Agency_Address','Amount_generated','Year','Receipt','Department');
      ...
      //  $crud->unset_export();
       ...
       // $crud->where('Event_Name','Consultancy Service');
        ...
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Consultancy</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Receipt','Evidence2','Evidence3','Status','Department');}
        $output=$crud->render();
        $this->load->view('depart');
...
function Cluster(){
       ...
       $crud->field_type('Status','hidden',"");
        ...
        $result['title']="<html><h2><center> <b> Cluster Meetings (Cluster of Departments) </b></h2></html>";
$this->load->view('adminsam1',$result);
      ...
      //  $crud->unset_print();
       ...
       // $crud->where('Event_Name','Cluster Meeting');
        ...
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Cluster Meetings (CLuster of Departments)</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Certificate','Evidence2','Evidence3','Status','Department');}
        $output=$crud->render();
        $this->load->view('depart');
...
$this->load->view('adminsam1',$result);
      ...
      //  $crud->unset_print();
       ...
       // $crud->where('Event_Name','Cluster Meeting');
        ...
        $crud->set_field_upload('Evidence2','assets/uploads/files/Clustermeet');
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Cluster Meetings (Cluster of Colleges)</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Certificate','Evidence2','Evidence3','Status','Department');}
        $output=$crud->render();
        $this->load->view('depart');
     ...
     public   function research(){
        ...
        $crud->field_type('Status','hidden','');
         ...
         $crud->set_rules('Year','Year','callback_check_year1[Year]');
        // $crud->unset_print();
        $_SESSION['CustomTitle']='<br><h3> <center> '.$Department.' </center><br><br>Staff - Research Guidance</h3>';     
        //$crud->set_rules('Date','Date','callback_check_dates[Date]');
       //$crud->where('Status','Approved');
        // $crud->unset_export();
        if($Department=='')
 ...
 public function extension()
    {
      ...
      if($Department=='')
        ...
        //$crud->unset_export();
		$crud->columns('Nature_of_Activity','Source_of_Fund','Venue','Date','No_of_Students','No_of_Faculty','Objective','Impact','Target_Group','Description','Amount_Spent','certificate','Evidence2','Evidence3');
        $crud->fields('Department','Nature_of_Activity','Source_of_Fund','Venue','Date','No_of_Students','No_of_Faculty','Target_Group','Objective','Impact','Description','No_of_Participants','Amount_Spent','certificate','Evidence2','Evidence3');
        $crud->required_fields('Department','Nature_of_Activity','Department','Venue','Date','No_of_Students','No_of_Participants','No_of_Faculty','Target_Group','Source_of_Fund','Amount_Spent','certificate');
        $crud->where('Department',$Department);
        $crud->set_field_upload('certificate','assets/uploads/files/ExtensionAct');
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Extension Activities</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Certificate','Evidence2','Evidence3','Status','Department');}
        $output=$crud->render();
        $this->load->view('depart');
    ...
    public function shield()
    {
        ...
        $this->load->view('depart');
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Overall Shields</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Certificate','Evidence2','Evidence3','Status','Department');}
        $output=$crud->render();
    ...
    public function field_visit()
    {
        ...
        $crud->field_type("Department",'hidden',$Department);
        $result['data']=$this->selection_model->disp1($Staff_ID);
        $result['title']="<html><h2><center> <b> Field Visits </b></h2></html>";
        $this->load->view('adminsam1',$result);
        ...
        $crud->required_fields('Date','Staff_Incharge','Institude_Address','No_of_Students','Class','Knowledge_gained','Certificate','Department');
        $crud->set_subject('Field Visit');
        $crud->where('Department',$Department);
		$crud->display_as('Institude_Address','Detail of Industry');
		$crud->columns('Date','Class','Institude_Address','No_of_Students','Certificate','Evidance2','Evidance3');
        //$crud->unset_print();
        //$crud->unset_export();
        ...
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Field Visit</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Certificate','Evidance2','Evidance3','Status','Department');}
        $output=$crud->render();
 $this->load->view('depart');
        $this->load->view('table',$output);
}
...
public function MoU_signed()
    ...
    //$crud->set_rules('To_Date','To_Date','callback_check_dates2[To_Date]');
		...
		$crud->columns('Date','Institute_Company','Purpose','Duration','Certificate','Evidence2','Evidence3');
      ...
      //  $crud->unset_print();
        ...
        $this->load->view('depart');
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>MoU Signed</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Certificate','Evidence2','Evidence3','Status','Department');}
        $output=$crud->render();
    ...
    public function alumni()
    {
        ...
        $crud->columns('Date','Detail_of_Alumni','Type_of_Program','Certificate','Evidence2','Evidence3');
      ...
      //  $crud->unset_print();
        ...
        $this->load->view('depart');
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Alumni Intraction</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Certificate','Evidence2','Evidence3','Status','Department');}
        $output=$crud->render();
...
public function specialprg()
    {
       ...
       $crud->required_fields('Department','Year','Date','Type_of_Program','Title_of_Course','Duration','Strength','Incharge_Faculty','Certificate');
       $crud->columns('Department','Date','Type_of_Program','Title_of_Course','Duration','Strength','Incharge_Faculty','Certificate','Evidence2','Evidence3');
        ...
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Special Programs</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Certificate','Evidence2','Evidence3','Status','Department');}
        $output=$crud->render();
        $this->load->view('depart');
    ...
    public function Dept()
    {
        ...
        $crud->set_table('department_activity');
        $crud->field_type('Level','dropdown',array("Internal"=>'Internal',"District"=>"District","Zonal"=>"Zonal","State"=>'State',"National"=>"National","International"=>'International'));
        $crud->field_type('Event','dropdown',array('Seminar'=>'Seminar','Workshop'=>'Workshop','Conference'=>'Conference','FDP'=>'FDP','Guest Lecture'=>'Guest Lecture','Inter - Collegiate Meet'=>'Inter - Collegiate Meet','others'=>'others'));
        //$crud->set_theme('datatables');
        $crud->unset_add();
        $crud->set_rules('Date','Date','callback_check_dates1[Date]');
    $crud->set_rules('To_Date','To_Date','callback_check_dates2[To_Date]');
       ...
       // $crud->unset_print();
        //$crud->unset_export();
        $crud->columns('Date','To_Date','Level','Event','Nature_of_program','Source_of_Funding','Amount','Name_of_Speaker','Certificate','Evidence2','Evidence3');
        ...
        $this->load->view('depart');
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Department Organized Events</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Evidence1','Evidence2','Certificate','Status','Department');}
        $output=$crud->render();
    ...
    public function book()
    {
       ...
       //$crud->where('Status','Approved');
        ...
        $crud->fields('Staff_ID','Staff_Name','Co_Author','Date','Status','Title_of_book','Type','Issn_isbn_for_proceeding','Year_of_publishing','Volume','Address_of_Publishers','Certificate','Evidence2','Evidence3',"Department");
        $crud->columns('Staff_ID','Staff_Name','Date','Title_of_book','Volume','Issn_isbn_for_proceeding','Year_of_publishing','Address_of_Publishers','Certificate','Evidence2','Evidence3',"Status");
        $crud->set_read_fields('Title_of_book','Issn_isbn_for_proceeding','Year_of_publishing','Certificate','Evidence2','Evidence3');
        $crud->required_fields('Volume','Address_of_Publishers','Title_of_book','Date','Issn_isbn_for_proceeding','Year_of_publishing','Certificate','Staff_ID','Staff_Name','Department');
        ///$crud->where('Nature_of_Award','Best Paper Award');
        $crud->display_as('Issn_isbn_for_proceeding','ISBN');
        //$Staff_Name=$dat->Staff_Name;
        $crud->display_as('Co_Author','Co-author of our college');
        $crud->where('Type','book');
        $state = $crud->getState();
        if ($state == 'export' || $state == 'print') {
            $crud->columns('Staff_ID','Staff_Name','Co_Author','Date','Title_of_book','Volume','Issn_isbn_for_proceeding','Year_of_publishing','Publication_Address');
        }
        $crud->field_type('Type','hidden','book');
        $result['data']=$this->selection_model->disp1($Staff_ID);
        $_SESSION['CustomTitle']='<br><h3> <center> <br>'.$Department.' </center><br><br>Books Published by Staff</h3>';     
...
$this->load->view('adminsam1',$result); 
    ...
    public function journal()
    {
        ...
        $_SESSION['CustomTitle']='<h3> <center> <br>'.$Department.' </center><br><br>Papers in Journals by Staff</h3>';     
        //$crud->where('Status','Approved');
       //$crud->set_rules('Year','Year','callback_check_year1[Year]');
       $crud->field_type('Mentioned','dropdown',array('yes'=>'yes','no'=>'no'));
...
$this->load->view('staff');
$crud->field_type('Level','dropdown',array("Internal"=>'Internal',"District"=>"District","Zonal"=>"Zonal","State"=>'State',"National"=>"National","International"=>'International'));
        $crud->fields('Staff_ID','Staff_Name','Year_Date','Status',"Department",'Name_of_Journal','Title_of_Paper','Level','ISSN_ISBN','Indexed','Page_No','Impact_Factor','Scopus','UGC','Organization','Mentioned','Certificate','Evidence2','Evidence3');
        $crud->columns('Staff_ID','Staff_Name','Name_of_Journal','Year_Date','Title_of_Paper','ISSN_ISBN','Indexed','Page_No','Impact_Factor','Scopus','UGC','Organization','Mentioned','Certificate','Evidence2','Evidence3','Status');
        $crud->set_read_fields('Staff_ID','Staff_Name',"Department",'Name_of_Journal','Year_Date','Title_of_Paper','ISSN_ISBN','Indexed','Page_No','Impact_Factor','Scopus','UGC','Organization','Mentioned','Certificate','Evidence2','Evidence3');
        $crud->required_fields('Staff_ID','Staff_Name',"Department",'Name_of_Journal','Year_Date','Title_of_Paper','Level','ISSN_ISBN','Indexed','Page_No','Scopus','UGC','Organization','Mentioned','Impact_Factor','Certificate');
       $crud->display_as('Page_No','Volume (Issue) and Page No.');
        $state=$crud->getState();
       //if($)
       $state = $crud->getState();
       if ($state == 'export' || $state == 'print') {
        $crud->columns('Staff_ID','Staff_Name','Name_of_Journal','Year_Date','Title_of_Paper','ISSN_ISBN','Indexed','Page_No','Impact_Factor');
       }
        $crud->display_as('Year_Date','Date');
        //$dat=$res[0];
        //$Staff_Name=$dat->Staff_Name;
...
//$crud->columns('Batch','Trends_of_progression','No_of_students');
      //  $crud->unset_print();
        ...
        //$crud->set_field_upload('Certificate','assets/uploads/files/Department');
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Higher Studies</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Evidence1','Evidence2','Evidence3','Status','Department');}
        $output=$crud->render();
                //$this->load->view('staff');
...
//$crud->columns('Batch','Trends_of_progression','No_of_students');
      //  $crud->unset_print();
        ...
        //$crud->set_field_upload('Certificate','assets/uploads/files/Department');
        $state=$crud->getState();
        $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Scholarships Received</h3>';     
        if ($state == 'export' || $state == 'print') {
         $crud->unset_columns('Evidence','Evidence2','Evidence3','Status','Department');}
        $output=$crud->render();
                //$this->load->view('staff');
...
$this->load->view('adminsam1',$result);
        ...
        $crud->set_rules('year','year','callback_check_year[year]');
          ...
          //  $crud->display_as('Title_of_Paper','Title of the Paper (if Presented)');
            ...
            $_SESSION['CustomTitle']='<br><h3> <center>'.$Department.' </center><br><br>Others</h3>';     
          $state=$crud->getState();
            if ($state == 'export' || $state == 'print') {
             $crud->unset_columns('Certificate','Evidence2','Evidence3','Status','Department');}    
            $output=$crud->render();
...
public function AnnexureIX()
    {
        ...
        $crud->set_field_upload('Evidence3','assets/uploads/files/project');
        $crud->set_field_upload('Evidence2','assets/uploads/files/project');
        $_SESSION['CustomTitle']='<br><h3> <center><br>'.$Department.' </center><br><br>Students Publications </h3>';
        $output=$crud->render();
        $result['data']=$this->selection_model->disp1($Staff_ID);
...
public function refresher_orientation()
    {
        ...
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
        $_SESSION['CustomTitle']='<br><h3> <center>Appraisal <br>'.$Department.' </center><br><br>Refresh/Orientation Courses </h3>';
    ...
    $crud->set_field_upload('Evidence2','assets/uploads/files/project');
    $crud->set_field_upload('Evidence3','assets/uploads/files/project');
    $crud->columns('Staff_Name','Name_of_Course','Duration_From','Place');
$output=$crud->render();
        $this->load->view('staff');
