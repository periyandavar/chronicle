<?php
class Appraisal3 extends CI_Controller
{  
...
$result['title'] ="<html><h2><center><b>Papers Published by Staff</b></h2></html>";
        $crud->columns('Staff_Name','Title_of_Paper','Name of the journal with Volume, issue & Page Nos.','Level','Year_Date');
       ...
       $this->load->view('adminsam2',$result);
        ...
        $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br>Publications by Staff Members</h3>';
        $state=$crud->getState();
...
public function consultancy_service()
{
    ...
    $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br>Consultancy Service</h3>';
        $crud->columns('Year','Consultancy Services Offered','Amount_generated','Date');
    ...
    public function overall_shield()
    {
        ...
        $crud->unset_delete();
        $crud->unset_read();
        $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br>Overall Shields</h3>';
        $output=$crud->render();
        $result['data']=$this->selection_model->disp1($Staff_ID);
...
public function add1($value, $row)
{
    ...
    public function work()
    {
        ...
        $crud->where('Department',$Department);
        //$crud->where('Event','Overall shield');
        $crud->columns('Staff_ID','Staff_Name','UG_Gain','UG_Loss','[UG] (+) or (-) ','PG_Gain','PG_Loss','[PG] (+) or (-) ');
       ...
       // $crud->display_as('POMU','[UG] (+) or (-) ');
        ...
        //$crud->unset_delete();
        //$crud->unset_read();
        $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br>Work Adjustment</h3>';
        $output=$crud->render();
        $result['data']=$this->selection_model->disp1($Staff_ID);
 ...
 $this->load->view('adminsam2',$result);
    ...
    public function app()
    {
        ...
        $crud->columns('Staff_Name','Title_of_Program','Date');
        //$crud->where('Status','Approved');
        //$crud->fields('Department','Staff_ID','Staff_Name','UG_Gain','UG_Loss','PG_Gain','PG_Loss');
        //$crud->required_fields('Department','Staff_ID','Staff_Name','UG_Gain','UG_Loss','PG_Gain','PG_Loss');
        ...
        $crud->unset_delete();
        $crud->unset_read();
        $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br>Mobile App Development</h3>';
...
$crud->unset_delete();
$crud->unset_read();
$_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br>Extension Activity';
$crud->callback_column('Nature of Extension activity carried out',array($this,'act'));
        $crud->columns('Date','Nature of Extension activity carried out','Impact');
    ...
    public function seminar1()
    {
        ...
        $crud->columns('Staff_Name','Title_of_Paper','Details','From_Date');
...
//        $crud->set_field_upload('Certificate','assets/uploads/files/Seminar');
  ...
  $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br> Papers Presented by Staff Members in Seminar/Symposium/Conference </h3>';
  $state=$crud->getState();
        if ($state == 'export' || $state == 'print') {
...
//        $crud->set_field_upload('Certificate','assets/uploads/files/Seminar');
  //      $crud->display_as('Name_of_Journal','Name of  the Journal');
  $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br> Staff Attended Seminars/Symposium/Conferences/Workshops </h3>';
  $state=$crud->getState();
  if ($state == 'export' || $state == 'print') {
      $crud->unset_columns('From_Date');
  }
...
//        $crud->set_field_upload('Certificate','assets/uploads/files/Seminar');
  //      $crud->display_as('Name_of_Journal','Name of  the Journal');
  $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br> Paper Presented by Staff Members in Seminar/Symposium/Conference/Workshop </h3>';
        $output=$crud->render();
        $this->load->view('Appraisal2');
        $this->load->view('showtable',$output);
    }
...
public function project_received()
    {
        ...
        $Staff_ID=$this->session->userdata('Staff_ID');
        $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br> Project Received</h3>';
    $result['data']=$this->selection_model->disp1($Staff_ID);
    $this->load->view('adminsam2',$result);
    ...
    $result['data']=$this->selection_model->disp1($Staff_ID);
    $this->load->view('adminsam2',$result);
    $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br> Research Guides</h3>';    
...
public function fund($C,$row)
{
    ...
    $this->load->view('adminsam2',$result);
       ...
       # $crud->or_where('Event','Workshop');
        $crud->where('Department',$Department);
        $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br> Department Organized Events';
        //$crud->where('Event','Seminar'AND'Workshop');
        #$crud->where('Event',['Seminar'],['Workshop']);
        #$crud->where('Event','Workshop');
        $crud->columns('Date','Title of the Seminar/Symposia/Conference','Name_of_Speaker','Source_of_Fund');
    ...
    $this->load->view('adminsam2',$result);
        ...
        $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br> MoU Signed</h3>';
       /* $crud->display_as('Staff_Name','Name of Teachers');
        $crud->display_as('Year','Year of Recognisation');
        $crud->display_as('Department','Discipline');*/
    ...
    $this->load->view('adminsam2',$result);
        $crud->where('Department',$Department);
        $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br>Extension Activities</h3>';
...
public function linkage_for_ojt()
    ...
    $result['data']=$this->selection_model->disp1($Staff_ID);
    $this->load->view('adminsam2',$result);
    $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br>Linkage for OJT</h3>';
        $this->load->view('Appraisal2');
...
$crud->unset_delete();
$crud->unset_read();
$_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal <br>'.$Department.' </center><br><br>Best Paper Awards by staff</h3>';
$result['title']= "<html><h2><center><b>Best Paper Awards by staff</b></h2></html>";
        //$crud->where('Department',$Department);
        $crud->where('Nature_of_Award','Best Paper Award');
        $crud->columns('Staff_Name','Place','From_Date');
...
$crud->unset_delete();
$crud->unset_read();
$_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br>Guest Lectures Delivered by staff</h3>';
  ...
  public function FDP()
    {
        ...
        $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br>Faculty Development Programs </h3>';
        if($Department=='')
        redirect('select_ctrl/header');
        //echo $_SESSION['CustomHeader'];
        $crud->field_type("Department",'hidden',$Department);
        $crud->unset_add();
        $result['title']= "<html><h2><center><b>FDP</b></h2></html>";
        $crud->columns('Date','Speaker','Organizing_Agency','From_Date');
        $state=$crud->getState();
        if ($state == 'export' || $state == 'print') {
            $crud->unset_columns('From_Date');        }
$output=$crud->render();
...
$Staff_ID=$this->session->userdata('Staff_ID');
    ...
    public function remed()
    {
        ...
        $crud->where('Department',$Department);
        //$crud->where('Event','Faculty Development Programme');
        $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br>Remedial Class </h3>';
        if($Department=='')
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
...
$Staff_ID=$this->session->userdata('Staff_ID');
    ...
    public function grant_applied()
    {
        ...
        $crud->where('Department',$Department);
        $crud->columns('Event_Name','Title','Fund_expected','Agency','Received','Date');
        $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br> Project Proposals Applied (Grant Applied)</h3>';
        if($Department=='')
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
...
$result['title']="<html><h2><center><b>Project Proposals Applied - Grant Applied</b></h2></html>";
    ...
    if($Department=='')
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
        $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br>Project Applied (student project) </h3>';
        $crud->where('Department',$Department);
    ...
    public function student_project()
    {
        ...
        $Staff_ID=$this->session->userdata('Staff_ID');
    $result['data']=$this->selection_model->disp1($Staff_ID);
    $result['title']= "<html><h2><center><b>Project Proposals Applied - Student Project </b></h2></html>";
    $this->load->view('adminsam2',$result);
    if($Department=='')
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
        $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br>Project Proposals Applied (student project) </h3>';
        $crud->where('Department',$Department);
    ...
    public function refresher_orientation()
    {
        ...
        redirect('select_ctrl/header');
        $crud->field_type("Department",'hidden',$Department);
        $_SESSION['CustomTitle']='<br><h3> <center>Department Appraisal<br>'.$Department.' </center><br><br>Refresh Orientation Programs </h3>';
