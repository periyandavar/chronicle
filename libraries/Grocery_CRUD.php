...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function _print_webpage($data)
	{
		...
		//$val= $sql->result();	
//		$string_to_print .=$_SESSION['head'].'<br>';
		//$_SESSION['CustomHeader']=($val[0]->detail); 
//        $this->load->model('selection_model');
		 $_SESSION['CustomHeader']="<center><h1>AYYA NADAR JANAKI AMMAL COLLEGE</h1><h5>".$_SESSION['head']."</h5>
         <h2>Sivakasi-West 626124</h2></center>";
		if(isset($_SESSION['CustomHeader']) && $_SESSION['CustomHeader']<>'') $string_to_print .= $_SESSION['CustomHeader'];
		if(isset($_SESSION['CustomTittle']) && $_SESSION['CustomTittle']<>'') $string_to_print .= $_SESSION['CustomTittle'];
		
		//$_SESSION['CustomTitle']=" <br>";
if(isset($_SESSION['CustomTitle']) && $_SESSION['CustomTitle']<>'') $string_to_print .= $_SESSION['CustomTitle'];
		if (count($data->list)==0)
		{
			$string_to_print .= "<center><b>NIL</b></center>";
		}
		else{
		$string_to_print .= '<table width="100%" cellpadding="0" cellspacing="0" ><tr>';
		$string_to_print .= "<th>S.No</th>";
		foreach($data->columns as $column){
		...
		{$string_to_print .= "<tr>";
			foreach($data->columns as $column){
			$string_to_print .= "<td><center>NULL</center></td>";}
			$string_to_print .= "</tr>";
		}
		else{
			$i=1;
		foreach($data->list as $num_row => $row){
			$string_to_print .= "<tr><td>".$i."</td>";
			foreach($data->columns as $column){
